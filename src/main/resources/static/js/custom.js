...
const postsDesigner = function (posts, rowClass) {
    ...
    for (let i = 0; i < posts.length; i++) {
        ...
        timeline += '<div class="row cmtt-action"><span class="ca-action"><i class="fa fa-heart"></i></span>'
            ...
            + '<form class="newComment" style="width: 100%;" method="GET" action="/comment">'
            + '<input type="hidden" name="commentToId" value="' + posts[i].id + '"/>'
            + '<input type="hidden" name="{_csrf.parameterName}" value="${_csrf.token}"/>'
            + '<input type="text" class="col-8" placeholder="enter comment" name="content"/>'
